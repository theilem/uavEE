...
void
IMUReader::notifyAggregationOnUpdate(Aggregator& agg)
{
...
IMUReader::run(RunStage stage)
{
    ...
    case RunStage::NORMAL:
    {
        ...
        {
            APLOG_ERROR << "Could not open port " << port.portName().toStdString() << ", IMUReader going into idle mode";
            idlemode_ = true;
            break;
        }
        ...
        {
            APLOG_ERROR << "Device at " << port.portName().toStdString() << " could not go into config mode, IMUReader going into idle mode";
            idlemode_ = true;
            break;
        }
        ...
        scheduler_.get()->schedule(std::bind(&IMUReader::readIMUData, this), Milliseconds(0), Milliseconds(10));
        break;
    }
    case RunStage::FINAL:
    {
...
IMUReader::readIMUData()
{
    ...
    lpacket.setDataFormat(XOM_Calibrated | XOM_Orientation | XOM_GpsPvt_Pressure, XOS_OrientationMode_Quaternion, 0);
    ...
    panTilt->panTiltIMUData_.imuData.attitude = Vector3(quaternion.w() * M_PI / 180, quaternion.x() * M_PI / 180, quaternion.y() * M_PI / 180);
    ...
    if(!panTilt->overrideGPS_)
    {
        ...
        double utmE, utmN;
        int zone;
        LLtoUTM(eWGS84, (double)gps_data.m_latitude / 1E7, (double)gps_data.m_longitude / 1E7, utmN, utmE, zone);
        panTilt->panTiltIMUData_.gpsData.position = Vector3(utmE, utmN, -(double)gps_data.m_height / 1E3);
        panTilt->panTiltIMUData_.gpsData.velocity = Vector3((double)gps_data.m_vele / 100, (double)gps_data.m_veln / 100, -(double)gps_data.m_veld / 100);
