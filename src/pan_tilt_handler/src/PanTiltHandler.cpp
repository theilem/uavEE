...
PanTiltHandler::PanTiltHandler() : overrideGPS_(false), overrideTarget_(false), idlemode_(true), sendOutput_(false), target(0, 0, M_PI / 2, M_PI)
{
...
PanTiltHandler::run(RunStage stage)
{
	...
	{
		pidCascade_ = std::make_shared<PanTiltPIDCascade>(&panTiltIMUData_, &aircraftSensorData_, &target, &output);
		...
		//TODO set IDC
		//        SerialIDCParams params(arduinoPath_, 115200, "\r");
		...
		//        }
		idlemode_ = false;
		scheduler_.get()->schedule(std::bind(&PanTiltHandler::calculateControl, this), Milliseconds(0), Milliseconds(10));
		break;
	}
	case RunStage::FINAL:
		break;
...
const PIDParametersMap &PanTiltHandler::getPIDMap() const
{
...
{
	return pidCascade_->tunePID(static_cast<PIDs>(tunePID.pid), tunePID.params);
}
...
PanTiltHandler::processAircraftSD(const simulation_interface::sensor_data& sd)
{
	...
	if(!overrideTarget_)
	{
		...
		target.pitch = atan2(aircraftSensorData_.position.z - panTiltIMUData_.gpsData.position[2], r);
		//target.pitch += M_PI;
		...
		APLOG_TRACE << "x: " << x << " y: " << y << " Target Heading: " << target.heading * 180 / M_PI << " Target Pitch: " << target.pitch * 180 / M_PI;
	}
}
...
PanTiltHandler::calculateControl()
{
	...
	std::stringstream ss;
	ss << (uint32_t)(1500 + output.panOut * 500) << " " << (uint32_t)(1500 - output.tiltOut * 500);
	//    std::string str;
