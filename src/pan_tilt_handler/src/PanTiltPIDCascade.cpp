...
PanTiltPIDCascade::configure(const boost::property_tree::ptree& config)
{
    ...
    for(auto it : pidConfig)
    {
		...
		tunePID(static_cast<PIDs>(pid->second), params);
		pidParams_.insert(std::make_pair(static_cast<PIDs>(pid->second), info));
    }
    return true;
}

bool
PanTiltPIDCascade::tunePID(PIDs pidIndicator, const Control::PID::Parameters& params)
{
...
std::map<PIDs, PIDStatus> PanTiltPIDCascade::getPIDStatus()
{
	std::map<PIDs, PIDStatus> status;
	status.insert(std::make_pair(static_cast<PIDs>(PanTiltPIDs::HEADING), headingPID_->getStatus()));
	status.insert(std::make_pair(static_cast<PIDs>(PanTiltPIDs::PITCH), pitchPID_->getStatus()));
	status.insert(std::make_pair(static_cast<PIDs>(PanTiltPIDs::HEADINGRATE), headingRatePID_->getStatus()));
	status.insert(std::make_pair(static_cast<PIDs>(PanTiltPIDs::PITCHRATE), pitchRatePID_->getStatus()));
    return status;
}
...
const PIDParametersMap &PanTiltPIDCascade::getPIDParamMap() const
{
    return pidParams_;
}
