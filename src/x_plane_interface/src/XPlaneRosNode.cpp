...
void
XPlaneRosNode::notifyAggregationOnUpdate(Aggregator& agg)
{
...
XPlaneRosNode::run(RunStage stage)
{
    ...
    case RunStage::NORMAL:
    {
        sensorDataPublisher_ =
            nodeHandle_->advertise<simulation_interface::sensor_data>("sensor_data", 20);

        actuationSubscriber_ = nodeHandle_->subscribe("actuation", 20,
                               &XPlaneRosNode::actuate, this);
        auto scheduler = scheduler_.get();

        scheduler->schedule(
            std::bind(&XPlaneRosNode::getSensorData, this), Milliseconds(0), Milliseconds(1000 / sensorFrequency_));
        break;
    }
    default:
        break;
...
XPlaneRosNode::actuate(const simulation_interface::actuation& act)
{
    ...
    float throttle[] = {throt, throt, throt, throt, throt, throt, throt, throt};
