...
void
MapLogic::notifyAggregationOnUpdate(Aggregator& agg)
{
...
MapLogic::run(RunStage stage)
{
    ...
    {
        ros::NodeHandle nh;
        requestDataService_ = nh.serviceClient<radio_comm::request_data>("/radio_comm/request_data");
        break;
    }
    case RunStage::FINAL:
        break;
...
{
    return true; //radioComm_.get()->askForSafetyNet(); TODO fix
}

void
MapLogic::setSafetyBounds(const Rectangle& rect)
{
    safetyRect_ = rect;
}
