...
void
LayoutGenerator::notifyAggregationOnUpdate(Aggregator& agg)
{
...
void LayoutGenerator::changeLayout(WidgetLoader *wid, const QString &type, int rows, int cols)
{
...
LayoutGenerator::createLayout(const boost::property_tree::ptree& json, QWidget* parent)
{
    ...
    if (pm.add("config", config, false))
    {
        ...
        if (type == "quad_layout")
        {
            //QJsonObject nw = config["NW"].toObject();
            //QJsonObject ne = config["NE"].toObject();
            //QJsonObject sw = config["SW"].toObject();
            //QJsonObject se = config["SE"].toObject();
            boost::property_tree::ptree nw, ne, sw, se;
            //if(!config_pm.add("nw",nw,true)||!config_pm.add("ne",ne,true)||!config_pm.add("sw",sw,true)||!config_pm.add("se",se,true)){
            //	APLOG_ERROR<<"quad layout missing items";
            //}
